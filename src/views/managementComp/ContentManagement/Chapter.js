...
const Chapter = () => {
  ...
  const getData = async (currentPage) => {
    ...
    const url = `https://dev-api.solvedudar.com/api/admin/chapter/data?page=${currentPage}`; // Replace with your API endpoint
    setLoading(true);
    try {
  ...
  const handleDeleteRole = async (id) => {
    try {
      const response = await fetch(`https://dev-api.solvedudar.com/api/admin/chapter/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
